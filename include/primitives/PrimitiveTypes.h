...
public:
    PrimitiveTypePoint(sead::Vector3f trans, float size, sead::Color4f col) : PrimitiveTypeBase(PrimitiveTypes::PRIM_POINT) {
        mTranslation = trans;
        mSize = size;
        mColor = col;
    }
...
public:
    PrimitiveTypeAxis(sead::Vector3f trans, float size) : PrimitiveTypeBase(PrimitiveTypes::PRIM_AXIS) {
        mTranslation = trans;
        mSize = size;
    }
...
public:
    PrimitiveTypeArea(const char* gName, sead::Color4f frame, sead::Color4f fill) : PrimitiveTypeBase(PrimitiveTypes::PRIM_AREA) {
        mGroupName = gName;
        mFrameColor = frame;
        mFillColor = fill;
    }
...
public:
    PrimitiveTypeTriangle(al::Triangle tri, sead::Color4f color) : PrimitiveTypeBase(PrimitiveTypes::PRIM_TRIANGLE) {
        mTriangle = tri;
        mColor = color;
    }
...
enum HitSensorRenderTypes {
    ...
    HitSensorType_ALL = HitSensorType_ATTACK | HitSensorType_EYE | HitSensorType_NPC | HitSensorType_TRAMPLE | HitSensorType_BIND |
                        HitSensorType_ENEMYBODY | HitSensorType_MAPOBJ | HitSensorType_PLAYERALL | HitSensorType_HOLDOBJ | HitSensorType_MISC
};
...
public:
    PrimitiveTypeBezierCurve(al::BezierCurve* curve, u32 percision, sead::Color4f color) : PrimitiveTypeBase(PrimitiveTypes::PRIM_BEZIER_CURVE) {
        mCurve = curve;
...
public:
    PrimitiveTypeRail(al::Rail* rail, u32 percision, sead::Color4f color) : PrimitiveTypeBase(PrimitiveTypes::PRIM_RAIL) {
        mRail = rail;
        mPercision = percision;
        mColor = color;
    }
