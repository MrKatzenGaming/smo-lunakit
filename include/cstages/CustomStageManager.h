...
public:
    ...
    const char* getEntryName(int resIdx, int catIdx, int entIdx) {
        return mStageResources.at(resIdx)->mCategories.at(catIdx)->mEntries.at(entIdx)->getName();
    }

    bool getEntryValid(int resIdx, int catIdx, int entIdx) {
        return mStageResources.at(resIdx)->mCategories.at(catIdx)->mEntries.at(entIdx)->isStageExist();
    }
