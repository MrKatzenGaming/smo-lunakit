...
struct HashArray {
    ...
    constexpr HashArray(const std::array<const char*, Length>& strings) : m_Hashes() {
        /* Hash and store all provided strings. */
        for (size_t i = 0; i < strings.size(); i++)
            m_Hashes[i] = {crc32::HashStr(strings[i]), i};

        /* Sort by hashes. */
        std::sort(m_Hashes.begin(), m_Hashes.end(), [](auto& left, auto& right) { return left.first < right.first; });
    }

    constexpr std::int64_t FindIndex(const std::string_view& str) const {
