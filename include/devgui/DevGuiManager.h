...
public:
    ...
    bool tryOpenKeyboard(uint16_t maxChars, PopupKeyboardType keyType, const char** output, bool* isKeyboardOpen) {
        return mPopupKeyboard->tryOpenKeyboard(maxChars, keyType, output, isKeyboardOpen);
    }
    ...
    float* getCurrentScreenSizeMulti() {
        return nn::oe::GetOperationMode() ? &mScreenSizeDocked : &mScreenSizeHandheld;
    }  // Get the current scale modifier for docked mode
