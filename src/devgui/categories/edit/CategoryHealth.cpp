...
CategoryHealth::CategoryHealth(const char* catName, const char* catDesc, sead::Heap* heap) : CategoryBase(catName, catDesc, heap) {}
...
void CategoryHealth::updateCatDisplay() {
    ...
    if (ImGui::Checkbox("Edit Health", &mIsOverride) && mHitData) {
        mTargetHealth = mHitData->mCurrentHealth;
    ...
    if (isInStageScene() && ImGui::Button("Get Life Up Heart"))
        holder->getGameDataFile()->getPlayerHitPointData()->getMaxUpItem();
}
