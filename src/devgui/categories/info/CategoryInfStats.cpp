...
CategoryInfStats::CategoryInfStats(const char* catName, const char* catDesc, sead::Heap* heap) : CategoryBase(catName, catDesc, heap) {}
...
void CategoryInfStats::updateCatDisplay() {
    ...
    jumpCount = rs::getPlayerJumpCount(stageScene->mGameDataHolder);
    throwCapCount = rs::getPlayerThrowCapCount(stageScene->mGameDataHolder);
    ...
    totalCoinNum = rs::getTotalCoinNum(stageScene->mGameDataHolder);
