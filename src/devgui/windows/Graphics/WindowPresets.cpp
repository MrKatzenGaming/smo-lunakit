...
WindowPresets::WindowPresets(DevGuiManager* parent, const char* winName, bool isActiveByDefault) : WindowBase(parent, winName, isActiveByDefault) {}
...
bool WindowPresets::tryUpdateWinDisplay() {
    ...
    if (preset->mIsOverrideSky)
        drawDropdown("Sky", skyList, skyList, preset->mTotalSky, &preset->mSky);
...
void WindowPresets::drawDropdown(const char* header, const char* options[], const char* translatedOptions[], const int totalOptions,
                                 const char** output) {
    if (ImGui::BeginCombo(header, *output, mComboFlags)) {
        for (int n = 0; n < totalOptions; n++) {
