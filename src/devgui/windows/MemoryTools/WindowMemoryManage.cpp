...
void WindowMemoryManage::drawSimpleHeapView() {
    ...
    if (scene) {
        ...
        if (kit) {
            al::LiveActorGroup* group = kit->mLiveActorGroupAllActors;
...
void WindowMemoryManage::drawComplexHeapTreeItem(sead::Heap* heap) {
    ...
    bool expanded = ImGui::TreeNodeEx(heap->getName().cstr(),
                                      ImGuiTreeNodeFlags_NoTreePushOnOpen | (hasNoChildren ? ImGuiTreeNodeFlags_Bullet : ImGuiTreeNodeFlags_None));
