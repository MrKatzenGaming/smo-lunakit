...
void DevGuiSaveData::read() {
    ...
    imgui::updateScale(nn::oe::GetOperationMode(), *mParent->getScreenSizeMultiDocked(), *mParent->getScreenSizeMultiHandheld());
    ...
    if (root.isExistKey("FavActorBrowser")) {
        ...
        for (u32 i = 0; i < favs.getSize(); i++) {
            ...
            sead::FormatFixedSafeString<0x40> nameString("%s", name);
