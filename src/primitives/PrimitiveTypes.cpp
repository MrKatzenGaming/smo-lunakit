...
#include "al/Library/Area/AreaObjUtil.h"
...
void PrimitiveTypeArea::render() {
    ...
    for (int i = 0; i < group->mCount; i++) {
        al::AreaObj* area = group->mBuffer[i];
        const char* shapeType;
        al::tryGetAreaObjStringArg(&shapeType, area, "ModelName");

        sead::Vector3f scale = area->mAreaShape->mScale;
        sead::Vector3f pos;
        pos.x = area->mAreaTR.m[0][3];
        pos.y = area->mAreaTR.m[1][3];
        pos.z = area->mAreaTR.m[2][3];
        renderer->setModelMatrix(area->mAreaTR);
...
void PrimitiveTypeTriangle::render() {
    ...
    renderer->drawLine(mTriangle.mPositions[0], mTriangle.mPositions[1], mColor);
    renderer->drawLine(mTriangle.mPositions[1], mTriangle.mPositions[2], mColor);
    renderer->drawLine(mTriangle.mPositions[2], mTriangle.mPositions[0], mColor);

    renderer->drawSphere4x8(mTriangle.mPositions[0], 9.f, mColor);
    renderer->drawSphere4x8(mTriangle.mPositions[1], 9.f, mColor);
    renderer->drawSphere4x8(mTriangle.mPositions[2], 9.f, mColor);
}
