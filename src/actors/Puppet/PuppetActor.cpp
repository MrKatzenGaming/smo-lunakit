...
#include "custom/al/actor/ActorActionKeeper.h"
...
void PuppetActor::init(const al::ActorInitInfo& initInfo) {
    typedef PlayerCostumeInfo* initMarioModelCommon(al::LiveActor * player, const al::ActorInitInfo& initInfo, const char* bodyName,
                                                    const char* capName, int subActorNum, bool isDemo, al::AudioKeeper* audioKeeper,
    ...
    normalModel->mActionKeeper->mPadAndCamCtrl->mRumbleCount = 0;  // set rumble count to zero so that no rumble actions will run
