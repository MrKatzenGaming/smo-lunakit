...
struct PACKED BsdBufferConfig {
    ...
    ulong tcp_tx_buf_max_size =
        0x30000;  ///< Maximum size of the TCP transfer (send) buffer. If it is 0, the size of the buffer is fixed to its initial value.
    ulong tcp_rx_buf_max_size =
        0x30000;                     ///< Maximum size of the TCP receive buffer. If it is 0, the size of the buffer is fixed to its initial value.
...
s32 SendTo(int, const void*, ulong, int, const sockaddr*, uint);
s32 Accept(int, sockaddr*, uint*);
s32 Bind(int, const sockaddr*, uint);
...
const char* InetNtop(int af, const void* src, char* dst, uint size);
...
Result Initialize(const nn::socket::Config&);
s32 Finalize();
s32 GetAddrInfo(const char*, const char*, struct addrinfo const*, addrinfo**);
...
s32 GetNameInfo(const sockaddr*, uint, char*, uint, char*, uint, int);
s32 GetNameInfo(const sockaddr*, uint, char*, uint, char*, uint, int, int);
...
hostent* GetHostByAddr(const void*, uint, int);
hostent* GetHostByAddr(const void*, uint, int, int);
